...
import styles from '@/app/login/auth-form/AuthForm.module.scss';
interface IToast {
  open: boolean;
...
export default function ToastComponent({ onOpen, open, children }: IToast) {
  return (
    <Toast.Provider swipeDirection="right">
      <Toast.Root
        open={open}
        onChange={() => onOpen}
        className="bg-red-600  rounded-md shadow-[hsl(206_22%_7%_/_35%)_0px_10px_38px_-10px,_hsl(206_22%_7%_/_20%)_0px_10px_20px_-15px] p-[15px] grid [grid-template-areas:_'title_action'_'description_action'] grid-cols-[auto_max-content] gap-x-[15px] items-center data-[state=open]:animate-slideIn data-[state=closed]:animate-hide data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=cancel]:translate-x-0 data-[swipe=cancel]:transition-[transform_200ms_ease-out] data-[swipe=end]:animate-swipeOut"
      >
        <Toast.Description className={'text-white'} asChild>
          Toast///
        </Toast.Description>
